...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the Athena component tagger Vite plugin and its dependencies.",
...
async function applyComponentTagger(appPath: string) {
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[athena] add Athena component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[athena] add Capacitor for mobile app support",
    });
