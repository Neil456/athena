  ...
  try {
    ...
    if (hasChanges) {
      ...
      let message = chatSummary
        ? `[athena] ${chatSummary} - ${changes.join(", ")}`
        : `[athena] ${changes.join(", ")}`;
      ...
      if (uncommittedFiles.length > 0) {
        ...
        try {
          commitHash = await gitCommit({
            path: appPath,
            message: message + " + extra files edited outside of Athena",
            amend: true,
          });
          logger.log(
            `Amend commit with changes outside of Athena: ${uncommittedFiles.join(", ")}`,
          );
        } catch (error) {
          // Just log, but don't throw an error because the user can still
          // commit these changes outside of Athena if needed.
          logger.error(
            `Failed to commit changes outside of Athena: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
